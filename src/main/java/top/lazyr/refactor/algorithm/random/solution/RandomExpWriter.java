package top.lazyr.refactor.algorithm.random.solution;
 ...
 * @created 2022/2/23
 */
public class RandomExpWriter {
    private static DecimalFormat format = new DecimalFormat("#0.0000");

    public static void write(List<Solution> solutions, int generation, RandomConfiguration configuration) {
        // 1、指标数据
        ...
        Graph originGraph = solutions.get(0).getOriginGraph();
        // 获取初始异味数据
        ...
        for (int i = 0; i < solutions.size(); i++) {
            ...
            Graph refactoredGraph = solutions.get(i).getPhenotype();
            // 异味消除率
    ...
    private static String buildRefactorInfo(List<Solution> paretoOptimalSolutions, int generation) {
        StringBuilder refactorInfo = new StringBuilder();
        refactorInfo.append(ConsolePrinter.createTitle(generation + "") + "\r\n");
        for (Solution paretoOptimalSolution : paretoOptimalSolutions) {
            List<String> refactors = paretoOptimalSolution.getRefactors();
    ...
    private static List<List<String>> buildFrontInfo(List<Solution> paretoOptimalSolutions, int generation) {
     ...
     * @return
     */
    private static List<List<Double>> solution2Vectors(List<Solution> solutions) {
